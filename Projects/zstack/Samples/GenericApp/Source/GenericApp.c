...
void GenericApp_HandleKeys( byte shift, byte keys )
{
  ...
  {
    if( BPSystemStatus == BP_OFFLINE ) // 离线-->寻找网络
    {
    ...
    else if( BPSystemStatus == BP_ONLINE) // 在线-->离线
    {
      // 发送断网消息给MSP430 
    ...
    else if ( BPSystemStatus == BP_FIND_NETWORK ) // 寻找网络-->离线
    {
      // Stop search network
...
void GenericApp_HandleNetworkStatus( devStates_t GenericApp_NwkStateTemp)
{
  ...
  {
      BPSystemStatus = BP_ONLINE;
      // 发送找到网络消息给MSP430
      bufferSend[1] = END_DEVICE;
      Serial_UartSendMsg(bufferSend,3);      
  }
  else if( BPSystemStatus != BP_OFFLINE) // Find network -- 1.coordinate lose 2.first connect to coordinate 
